...
open(FILE,"samples_to_remove.txt") or die "Can't open file: $!\n";
while ( <FILE> ) {
    chomp;
    @l = split(/\t/);
    $rem_smp{$l[0]} = 1;
...
open(FILE,"collapse.txt") or die "Can't open file: $!\n";
while ( <FILE> ) {
    chomp;
    @l = split(/\t/);
    $collapsed{$l[0]} = $l[1];
...
if($ARGV[1] eq "F"){
	open(msfile, $ARGV[0]) or die "Can't open file: $ARGV[0]\n";
	$fl = <msfile>;
	...
	open(msfile, $ARGV[0]) or die "Can't open file: $!\n";
	$fl = <msfile>;
...
open(FILE, "specificity_exclusions.txt") or die "Can't open file: $!\n";
while(<FILE>){
        chomp;
        @l = split(/\t/);
        $spec = "";
