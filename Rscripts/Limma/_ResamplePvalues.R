...
if(PREPROCESSED_ABUNDANCES){
  ...
  for(idx in 1:ncol(P_abundances)){
    ## cycle through all ratios
    value_name = colnames(input)[VAL_idx+idx-1]
    input_grouped = sqldf(str_join("select ",id_name,", avg(",value_name,") as 'average_score', count(*) as 'count' from input where ",value_name," != 0 group by ",id_name," order by ",id_name," asc"))
